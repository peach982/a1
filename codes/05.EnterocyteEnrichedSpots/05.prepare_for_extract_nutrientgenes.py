...
class generate:
	...
	def __init__ (self):
		...
		#self.path = path+'/'
		self.input1 = open('/data/tangyuanling/STsnRNA/forQinglin20251014/monocle23_time_course/ScaledData_allgenes_allenterocytes.txt','r')
		...
		self.output = open('/data/tangyuanling/STsnRNA/forQinglin20251014/monocle23_time_course/ScaledData_allgenes_allenterocytes.humansymbols.txt','w')
		#self.name = name
	...
	def generate (self):
	        ...
	        for line in self.input1:
	               ...
	               if i==0:
	                       ##headline=line
	                       print>>self.output,'humanname'+'\t'+'pigname'+'\t'+'\t'.join(parts[:])+'\t'+'type'

	               elif i!=0:
	                       if not(parts[0].startswith('ENSSSCG')):
	                               print>>self.output,parts[0]+'\t'+'\t'.join(parts[:])+'\t'+'direct'
	                               exclude[parts[0]]=0

	                       else:
	                               mydata[parts[0]]='\t'.join(parts[:])
