...
class generate:
	...
	def __init__ (self,path='/Lustre03/data/tangqianzi/Gut_unroll/outputs/'):
		...
		self.path = path
		self.jobfolder = 'runall_03stepjobs'
...
python /Lustre03/data/tangqianzi/Gut_unroll/outputs/03.filter_before_lognorm.py 6 '''+mysample

	        print>>myoutput1,'/usr/bin/perl /Lustre01/tangqianzi/software/scripts/qsub-sgenew.pl --queue tangqianzi --maxjob 200 --lines 2 --jobprefix step3 --convert no --resource nodes=1:ppn=1,mem=20g '+self.path+'/'+self.jobfolder+'/alljobs_dump.sh'
